...
type AboutChapter = {
  title: string;
  info: InfoItem[] | AwardsItem[] | ExperienceItem[] | CredentialsItem[];
};
      ...
      <div className="container mx-auto h-full flex flex-col items-center xl:flex-row gap-x-6">
        ...
        >
          <div className="flex gap-x-4 xl:gap-x-8 mx-auto xl:mx-0 mb-4">
            {aboutChapters.map((item, itemIndex) => {
              return (
                <div
                  key={itemIndex}
                  className={`${
          ...
          <div className="py-2 xl:py-6 flex flex-col gap-y-2 xl:gap-y-4 items-center xl:items-start">
            {aboutChapters[index].info.map((item, itemIndex) => {
              return (
                <div
                  key={itemIndex}
                ...
                >
                  {/* title */}
                  <div className="font-ligth mb-2 md:mb-0">{item.title}</div>
                  <div className="hidden md:flex">-</div>
                  <div>{(item as AwardsItem).stage}</div>
                  {/* icons */}
                  <div className="flex gap-x-4">
                    {(item as InfoItem).icons?.map((icon, iconIndex) => {
                      return (
